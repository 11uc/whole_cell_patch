...
class MultiPlot(Analysis):
	...
	def avePlotMultiProt(self, protocols, types, cells, stims, trials, aveLevel, 
			label1, label2, normWin, win, errorBar, magnify):
		'''
		Plotting raw data trace across type, cell, stimulation amplitude
		and trials. Cell and trials could be averaged. If average is taken 
		...
		Parameters
		----------
		protocols: array_like
			Protocols of trials from which will be plotted.
		types: array_like
			Types of cells to include. Include all the cells when empty.
		cells: array_like
			Ids of cells to include. Include all the cells when empty.
		...
		aveLevel: string
			...
			"none" -
				Plot individual trials without averaging.
		label1: string
			One of "type", "cell", "protocol", "stim", "trial", and "none". Part 1 of
			the label for each trace. If "none", nothing will be labeledl.
		label2: string
			One of "type", "cell", "protocol", "stim", "trial", and "none". Part 2 of
			the label for each trace, if "none", only part 1 will be used.
		normWin: list
			Of two scalars, time window of trace used as baseline to
			normalize the traces. Won't be applied if not valid.
		...
		trialTables = []
		for p in protocols:
			t = self.projMan.getTrialTable(p, cells, trials, types, stims)
			t["protocol"] = p
		...
		if aveLevel == "none":
			...
			grp = trialTable.groupby(["stim", "cell", "protocol"])
			for k, v in grp.groups.items():
				tTraces = []
				for c, t in trialTable.loc[v, ["cell", "trial"]].values:
					tr, sr, stim = self.projMan.loadWave(c, t)
				...
				if aveLevel == "trials":
					...
					if label1 == "type" or label1 == "stim" or label1 == "protocol":
						if (label2 == "type" or label2 == "stim" or 
								label2 == "protocol") and label2 != label1:
							labels.append("{0} {1}, {2} {3}".format(
									label1, trialTable.loc[v[0], label1],
									label2, trialTable.loc[v[0], label2]))
						else:
			...
			grp = trialTable.groupby(["type", "stim", "protocol"])
			for k, v in grp.groups.items():
				tTraces = []
				for c, t in trialTable.loc[v, ["cell", "trial"]].values:
					tr, sr, stim = self.projMan.loadWave(c, t)
				...
				if errorBar:
					if len(cv) > 2:
						...
						errors.append([])
				if label1 == "type" or label1 == "stim" or label1 == "protocol":
					if (label2 == "type" or label2 == "stim" or label2 == "protocol") \
							and label2 != label1:
						labels.append("{0} {1}, {2} {3}".format(
									label1, trialTable.loc[v[0], label1],
									label2, trialTable.loc[v[0], label2]))
		...
		if len(labels):
			...
			# xkcd_colors = list(mcd.XKCD_COLORS.keys())
			t10_colors = ['tab:cyan', 'tab:red', 'tab:blue', 'tab:orange', 
					'tab:green', 'tab:purple', 'tab:brown', 'tab:gray', 
					'tab:olive', 'tab:pink']
			colors = [t10_colors[d % len(t10_colors)] 
				for d in range(len(uniLabels))]
		if len(traces):
		...
		prof = [
			{"name": "Multi-trace plot",
				...
				"param": {"protocol": "protocol",
					...
					"errorBar": "bool",
					"magnify": "float"}},
			{"name": "Multi-trace multi-protocol",
				"pname": "mpPlotP",
				"foo": self.avePlotMultiProt,
				"param": {"protocols": "strl",
					...
					"trials": "intl",
					"aveLevel": "combo,cells,trials,none",
					"label1": "combo,type,cell,protocol,stim,trial,none",
					"label2": "combo,type,cell,protocol,stim,trial,none",
					"normWin": "floatr",
					"win": "floatr",
					"errorBar": "bool",
					"magnify": "float"}}
